...
def write_oskar(wd=None, metafits=None, srclist=None, oskar_uvfits_tag=None, time=None, band_num=None, 
                data_dir=None, telescope=None, time_int=None, ini_file=None, low_freq=None,
                flag_dipoles=None, cluster=None,retain_vis_file=None,retain_ini_file=None,
                oskar_gsm_file=False,oskar_gsm_SI=False):
    ...
    oskar_options = " --low_freq=%.1f --output_name=%s --time=%s --band_num=%s --debug --data_dir=%s" %(low_freq, oskar_uvfits_tag, time, band_num, data_dir)
    if srclist:
        ...
        exit()
    if metafits:
        oskar_options += ' --metafits=%s' %metafits
    if telescope:
    ...
    if oskar_gsm_SI:
        oskar_options += ' --oskar_gsm_SI=%s' %oskar_gsm_SI
    if oskar_gsm_file:
        oskar_options += ' --oskar_gsm_file=%s' %oskar_gsm_file
    
    out_file.write('time %s/SKAobs_oskar.py %s &\n' %(OSKAR_dir, oskar_options))
...
parser.add_option('--low_freq',default=100e+6,help='Lowest frequency band')
parser.add_option('--oskar_gsm_SI',default=False,help='SI iniit')
parser.add_option('--oskar_gsm_file',default=False,help='GSM file')
...
oskar_gsm_SI = options.oskar_gsm_SI
...
for band_num in band_nums:
    oskar_slurm = write_oskar(wd=wd, metafits=metafits, srclist=srclist, oskar_uvfits_tag=oskar_uvfits_tag, time=time, band_num=band_num, data_dir=output_dir,
        ...
        retain_ini_file=retain_ini_file,low_freq=low_freq,oskar_gsm_SI=oskar_gsm_SI,oskar_gsm_file=oskar_gsm_file)
