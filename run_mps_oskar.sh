...
for i in $DEVICE_1 $DEVICE_2; do
    mkdir $PWD/tmp/mps-$i $PWD/tmp/mps-log-$i
    export CUDA_VISIBLE_DEVICES=$i # SELECT GPU
    export CUDA_MPS_PIPE_DIRECTORY=$PWD/tmp/mps-$i # NAMED PIPES
    export CUDA_MPS_LOG_DIRECTORY=$PWD/tmp/mps-log-$i # LOGFILES
...
export CUDA_MPS_PIPE_DIRECTORY=$PWD/tmp/mps-$DEVICE_1
source $SCRIPT_1

export CUDA_MPS_PIPE_DIRECTORY=$PWD/tmp/mps-$DEVICE_2
source $SCRIPT_2
...
##########BP: Shutdown
for i in $DEVICE_1 $DEVICE_2; do
export CUDA_MPS_PIPE_DIRECTORY=$PWD/tmp/mps-$i # SELECT MPS DAEMON
echo quit | nvidia-cuda-mps-control # STOP MPS DAEMON
rm -rf $PWD/tmp/mps-$i $PWD/tmp/mps-log-$i
done
